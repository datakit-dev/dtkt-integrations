...
These examples depend on a running instance of the Mailpit integration package.

## Connections

Mailpit connections for use in examples.
...
Get a message by its ID (or latest).

```shell
dtkt call ExecuteAction \
  --conn mailpit \
  -f examples/action-service/execute-action/get-message/input.json
```

<img alt="get-message demo with VHS" src="./action-service/execute-action/get-message/vhs.gif" width="800" />
