...
func (_q *TaskQuery) QuerySchema() *ExtractionSchemaQuery {
...
func (_q *TaskQuery) WithSchema(opts ...func(*ExtractionSchemaQuery)) *TaskQuery {
	query := (&ExtractionSchemaClient{config: _q.config}).Query()
...
func (_q *TaskQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Task, error) {
	...
	if query := _q.withSchema; query != nil {
		if err := _q.loadSchema(ctx, query, nodes, nil,
			func(n *Task, e *ExtractionSchema) { n.Edges.Schema = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TaskQuery) loadSchema(ctx context.Context, query *ExtractionSchemaQuery, nodes []*Task, init func(*Task), assign func(*Task, *ExtractionSchema)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Task)
	for i := range nodes {
		if nodes[i].SchemaID == nil {
			continue
		}
		fk := *nodes[i].SchemaID
...
func (_q *TaskQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withSchema != nil {
			_spec.Node.AddColumnOnce(task.FieldSchemaID)
		}
	}
